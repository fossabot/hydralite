...
export function init(bot: Bot) {
  ...
  client.interactions.on("buttonInteraction", async (interaction) => {
    ...
    if (server.roles.find((v) => v.id == interaction.customId))
      toogleRole(
        interaction.customId,
        interaction.raw.member,
        await channel.messages.fetch(interaction.raw.message?.id)
      );
    interaction.respond({
      type: 6,
    });
...
async function update(message: Message) {
  const msg = await client.interactions.fetchMessage({
  ...
  msg.edit({
    ...
    content:
      "https://cdn.discordapp.com/attachments/843780743695171584/862763593509371964/hydralite_channel_header_roles.png",
  } as any);
}
...
async function toogleRole(type: string, user: Member, message: Message) {
  try {
    ...
    const member = await (
      await message.guild.members.fetch(user.user.id)
    ).fetch(true);

    if (!guild_role) return;
    if (guild_role.members.has(member.id)) member.roles.remove(guild_role);
    else member.roles.add(guild_role);

    addRecent(
      type,
      user.nick ?? user.user.username,
      message,
      !guild_role.members.has(member.id)
    );
  } catch (error) {
    console.log(error);
  }
...
function getButtons(guild: string) {
  ...
  const dev = new LinkButtonComponent(
    "https://hydraliteusercache.eliyah.repl.co/",
    {
  ...
  const buttons: ButtonComponent[] = server.roles.map((v) =>
    getButton(v.id, v.title)
  );
